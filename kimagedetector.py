...
def render_detected(overlay, detected,label, x1, y1, x2, y2, color=(0,255,0), label_format=None, font="sans-serif", font_size=12, line_width=2, scale=1):
    ...
    overlay.draw_clear(id='render_detected_roi')
    overlay.draw_rect(x1, y1, x2, y2,"white",line_width,True,id='render_detected_roi')
    overlay.draw_clear(id='render_detected_roi')
    overlay.draw_text(650, 20, "Total IN: "+label, font=dict(family="sans-serif", size=20, color="white"),fillcolor="black",padding=3, xanchor="left", id='render_detected_roi')

    #print(x1,y1,x2,y2)
    
    render_detected_roi(overlay, x1, y1, x2, y2,(255,255,0), font, font_size, line_width, scale)
...
#------------------------------------------------ROI------------------------------------------------------------------------------------------------------
def render_detected_roi(overlay,  x1, y1, x2, y2, color=(0,0,255), font="sans-serif", font_size=12, line_width=2, scale=1):
    html_color = f'rgb({color[0]},{color[1]},{color[2]}' 
    overlay.draw_rect(*[x1,y1,x2,y2], fillcolor="rgba(0,0,0,0)", line=dict(color=html_color, width=line_width), id='render_detected_roi')

    offset = int(line_width/2)
    xoffset = 0 if x1<offset else -offset
    if y1<font_size*2/scale:
        yoffset = 0 if y1<offset else -offset
        yanchor = "top"
    else:
        yoffset = offset
        yanchor = "bottom"
    ...
    #color_calc = color[0]*0.8+color[1]*1.4+color[2]*0.5
    #text_color = "white" if color_calc<350 else "black"
    overlay.draw_text(x1+xoffset, y1+yoffset, " ", font=dict(family=font, size=font_size, color="black"), fillcolor=html_color, xanchor="left", yanchor=yanchor, id='render_detected_roi')
    
    
def render_detected_rect(overlay,  x1, y1, x2, y2, color=(0,255,0),label_format=None, font="sans-serif", font_size=12, line_width=2, scale=1):
    overlay.draw_clear(id='render_detected_roi')
    overlay.draw_rect(x1, y1, x2, y2,"white",line_width,True,id='render_detected_roi')
    overlay.draw_clear(id='render_detected_roi')
    #print(x1,y1,x2,y2)
    
    render_detected_roi(overlay, x1, y1, x2, y2, color, font, font_size, line_width, scale)

    return overlay.out_draw()



def render_roi_image(image,  x1, y1, x2, y2, color=(0,0,255), x_offset=0, y_offset=0,  font_size=1.0, font_width=0, line_width=0):
    ...
    cv2.rectangle(image, (x1+x_offset, y1+y_offset), (x2+x_offset, y2+y_offset), color, line_width)
