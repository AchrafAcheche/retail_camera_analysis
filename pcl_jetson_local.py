...
def crossing_line_local():
    ...
    while True:
        try:
                    ...
                    if objectId not in tracked_objects.keys():
                        ...
                        if position != position_ini:
                            if position == 1:
                                ...
                                filename = "image_{}_{}_{}_{}_{}_face_in_{} ".format(bbox[0], bbox[1], bbox[2], bbox[3],
                                                                                     str(cameraID), str(t))
                            ...
                            if position == -1:
                                ...
                                down += 1
                                t = datetime.now().strftime(" %Y-%m-%d-%H-%M")
                                filename = "image_{}_{}_{}_{}_{}_face_out_{} ".format(bbox[0], bbox[1], bbox[2],
                                                                                      bbox[3], str(cameraID), str(t))
                                cv2.imwrite('output/' + filename + '.jpg', image)
            ...
            cv2.rectangle(image, (contourROC[0][0], contourROC[0][1]), (contourROC[2][0], contourROC[2][1]),
                          (0, 255, 0), 2)
            cv2.putText(image, 'in :' + str(inObject) + " out : " + str(outObject), (150, 400), cv2.FONT_HERSHEY_PLAIN,
                        2, (0, 0, 255), 2)
